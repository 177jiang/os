...
void ps2_kbd_init() {
    ...
    if (acpi_ctx->fadt.header.rev > 1) {
        ...
        if (!(acpi_ctx->fadt.boot_arch & IAPC_ARCH_8042)) {
            kprintf_error("No PS/2 controller detected.\n");
            // FUTURE: Some alternative fallback on this? Check PCI bus for USB controller instead?
            return;
        }
    } else {
        kprintf_("Outdated FADT used, assuming 8042 always exist.\n");
    }
    ...
    if (result != PS2_RESULT_TEST_OK) {
        kprintf_error("Controller self-test failed.");
        goto done;
    }
    ...
    if (result != 0) {
        kprintf_error("Interface test on port 1 failed.");
        goto done;
    }
    ...
    intr_setvector(PC_KBD_IV, intr_ps2_kbd_handler);
