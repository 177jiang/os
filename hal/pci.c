...
static struct list_header pci_devices;
...
void pci_probe_device(int bus, int dev, int funct) {
    ...
    *device = (struct pci_device){ .cspace_base = base,
    ...
    list_append(&pci_devices, &device->dev_chain);
}
...
void pci_print_device() {
    ...
    list_for_each(pos, n, &pci_devices, dev_chain) {
        kprintf_live("(B%xh:D%xh:F%xh) Dev %x:%x, Class 0x%x\n",
        ...
        for (size_t i = 1; i <= 6; i++) {
            ...
            if (!bar) continue;
            if (PCI_BAR_MMIO(bar)) {
                kprintf_live("\t BAR#%d (MMIO) %p [%d]\n",
                                i,
                                PCI_BAR_ADDR_MM(bar),
                                size);
            } else {
                kprintf_live("\t BAR#%d (I/O) %p [%d]\n",
                                i,
                                PCI_BAR_ADDR_IO(bar),
                                size);
            }
