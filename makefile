...
all-debug: O := -O
all-debug: CFLAGS := -g -std=gnu99 -ffreestanding $(O) $(W) -D__JYOS_DEBUG__
...
debug-qemu: all-debug
	...
	@qemu-system-i386 -s -S -kernel $(BIN_DIR)/$(OS_BIN) -d cpu_reset -monitor telnet::$(QEMU_MON_PORT),server,nowait &
	@sleep 1
